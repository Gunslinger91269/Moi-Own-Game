...
function draw(){
    ...
    if(frameCount%1200){
       ...
       for(var i = 0;i<circleGroup.length;i++){
        ...
        if(circleGroup[i].body.position.x){
...
function createBlock(){
    if(level===1){
      ...
      if(level===0){
        ...
        c1=new Ball(windowWidth,random(500,600),random(250,350))
        circleGroup.push(c1);
    ...
    function keyPressed(){
      if(keyCode===32){
        ...
        Matter.Body.applyForce(guy.body,guy.body.position,{x:85,y:-85})
      }
    }